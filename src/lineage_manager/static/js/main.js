...
import { BASE_URL, EVENT_ID_STORAGE_KEY } from "./app/config.js";
import { ApiClient } from "./app/services/api.js";
import { authClient } from "./auth.js";

const api = new ApiClient(authClient);
...
async function fetchNeighbors(kind, value) {
  return api.fetchNeighbors(kind, value);
}
...
function renderGraph(data) {
  ...
  ctxExpand?.addEventListener('click', async () => {
    ...
    if (isJob && !params.node_db_id) params.node_id = ctxTargetNode.data('label') || ctxTargetNode.data('id');
    if (!isJob && !params.node_db_id) params.table_name = ctxTargetNode.data('label') || ctxTargetNode.data('id');
  ...
  ctxSync?.addEventListener('click', async () => {
    ...
    const dbid = parseInt(id.slice(1), 10);
    try {
      await api.syncNode({ node_type: isJob ? 'job' : 'table', node_db_id: dbid });
      // After sync, expand 1 depth to pick up changes
      const params = { node_type: isJob ? 'job' : 'table', node_db_id: String(dbid), direction: 'both', depth: '1' };
      const payload = await api.expand(params);
...
async function renderTableTriggers(tableName, infoDiv) {
  try {
    const data = await api.fetchTableTriggers(tableName);
    ...
    infoDiv.querySelectorAll('.trigger-toggle').forEach(el => {
      el.addEventListener('change', async () => {
        ...
        try {
          await api.patchTableTrigger(tableName, jobId, want);
    ...
    bulkOff?.addEventListener('click', async () => {
      ...
      if (!ok) return;
      try {
        await api.bulkDisableTriggers(tableName);
...
async function expandFrom(node, direction) {
  try {
    ...
    const level = 1;
    if (type === 'job') {
      const data = await api.fetchNeighbors('job', node.data('label'), level, direction);
      mergeGraph(window.cy, data, node.id(), direction);
    } else {
      const tableName = node.data('full_name') || node.data('label');
      const data = await api.fetchNeighbors('table', tableName, level, direction);
      mergeGraph(window.cy, data, node.id(), direction);
    }
  } catch (e) { /* noop */ }
}
