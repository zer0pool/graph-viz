...
export class PanelController {
    constructor(apiClient) {
        ...
        this.triggerManager = null; // Will be initialized in init()
    ...
    renderTableDetails(node) {
        ...
        // Render triggers for this table
        this.renderTriggers(node);
    }
    ...
    async fetchJobOverview(jobId, requestId) {
        if (!jobId) return;
        try {
            const response = await this.api.fetchJobDetail(jobId);
            if (requestId !== this.jobOverviewRequestId) return;

            // Handle {status, result} wrapper or direct object
            const detail = (response.status === "success" && response.result) ? response.result : response;
            ...
            // Populate I/O links from upstreams/downstreams (root or properties)
            const upstreams = detail.upstreams || (detail.properties && detail.properties.upstreams) || [];
            const downstreams = detail.downstreams || (detail.properties && detail.properties.downstreams) || [];

            let inputs = Array.isArray(upstreams) ? upstreams.filter(u => u.type === 'table') : [];
            let outputs = Array.isArray(downstreams) ? downstreams.filter(d => d.type === 'table') : [];
            ...
            // Sync with graph relations if API is empty
