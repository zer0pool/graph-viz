...
class TestGraphSyncService:
    ...
    @pytest.fixture
    def service(self, mock_uow, mock_job_manager, mock_command_service, mock_query_service, mock_initializer):
    ...
    @pytest.mark.asyncio
    async def test_sync_job_from_manager_success(self, service, mock_job_manager, mock_command_service):
        lineage = SchedulingLineage(job_id="job1", name="Job 1", type="SELF-TYPE", status="ENABLED")
    ...
    @pytest.mark.asyncio
    async def test_sync_from_job_manager_delegates_to_initializer(self, service, mock_initializer):
    ...
    @pytest.mark.asyncio
    async def test_sync_multiple_jobs_success(self, service, mock_job_manager, mock_command_service):
        lineages = [
            SchedulingLineage(job_id="job1", name="Job 1", type="SELF-TYPE", status="ENABLED"),
            SchedulingLineage(job_id="job2", name="Job 2", type="SELF-TYPE", status="ENABLED")
        ]
        mock_job_manager.fetch_lineages_by_ids = AsyncMock(return_value=lineages)
        
        job_requests = [{"job_id": "job1", "type": "req"}, {"job_id": "job2", "type": "req"}]
    ...
    @pytest.mark.asyncio
    async def test_sync_multiple_jobs_partial_failure(self, service, mock_job_manager, mock_command_service):
        lineages = [
            SchedulingLineage(job_id="job1", name="Job 1", type="SELF-TYPE", status="ENABLED"),
            SchedulingLineage(job_id="job2", name="Job 2", type="SELF-TYPE", status="ENABLED")
        ]
