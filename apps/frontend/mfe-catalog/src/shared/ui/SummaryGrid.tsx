...
function MetricCard({ data }: { data: MetricData | any }) {
  ...
  return (
    <Card
      className={`group relative overflow-hidden border-slate-200/60 shadow-sm transition-all duration-300 hover:shadow-md hover:-translate-y-1 bg-gradient-to-br ${getGradient(data.type)}`}
    >
      <CardHeader className="pb-3 relative z-10">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-wider text-slate-500/80">
            <div
              className={`p-1.5 rounded-lg bg-white border border-slate-100 shadow-sm transition-transform group-hover:scale-110 group-hover:rotate-3`}
            >
      ...
      <CardContent className="relative z-10">
        ...
        {data.breakdown && data.breakdown.length > 0 && (
          <div className="mt-3 pt-2 border-t border-slate-100 flex flex-col">
            ...
            <div
              className={`flex flex-col gap-1.5 transition-all duration-300 ease-in-out origin-top overflow-hidden ${isExpanded ? "max-h-24 opacity-100 mt-1" : "max-h-0 opacity-0 pointer-events-none"}`}
            >
...
export function SummaryGrid({ metrics, cols = 5 }: SummaryGridProps) {
  ...
  return (
    <div
      className={`grid grid-cols-2 md:grid-cols-3 ${gridColsClass} gap-5 mb-8 animate-in fade-in slide-in-from-bottom-4 duration-700`}
    >
      {metrics.map((m, idx) => (
