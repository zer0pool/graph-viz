...
describe('useJobFilter hook', () => {
  ...
  it('should perform search and return jobs', async () => {
    const dummyJobs = [{ id: 'j1', displayLabel: 'One', config: { owner: 'o', projectId: 'p' }, stats: { lastRunStatus: 'RUNNING', updatedAt: '2026-03-01T00:00:00Z', duration: 123, progress: 0.42 } }];
    ...
    expect(mockApi.graphqlRequest).toHaveBeenCalledWith(expect.any(String), { filter: { searchTerm: 'foo' } });
  });
