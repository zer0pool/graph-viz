    ...
    <div className="p-6">
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm mt-8 animate-fade-in-up delay-300">
        ...
        <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <h3 className="font-semibold text-slate-700">Recently Finished Jobs (Total: {totalCount})</h3>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center bg-slate-200/50 p-1 rounded-lg border border-slate-200">
                ...
                <div className="relative ml-1">
                  ...
                  >
                    Custom {timeRange === "custom" && <span className="w-1 h-1 bg-blue-600 rounded-full"></span>}
                  </button>

                  {showCustomRange && (
                    <>
                      <div className="fixed inset-0 z-[75]" onClick={() => setShowCustomRange(false)} />
                      <div className="absolute top-full right-0 mt-2 w-72 bg-white border border-slate-200 shadow-2xl rounded-xl z-[80] p-4 animate-fade-in-down">
                         <div className="flex justify-between items-center mb-4">
                           ...
                           <X className="w-3.5 h-3.5 text-slate-400 cursor-pointer hover:text-slate-600" onClick={() => setShowCustomRange(false)} />
                         </div>
                        ...
                        <div className="p-1">
                          ...
                          >
                            ...
                            <span className="text-xs font-bold text-slate-700">
                              {tempVisibleColumns.length === 0 ? "Select all" : 
                               tempVisibleColumns.length === availableColumns.length ? `All ${availableColumns.length} selected` : 
                          ...
                          <div className="max-h-60 overflow-y-auto space-y-0.5 custom-scrollbar">
                            {availableColumns.map((col) => (
                              <label key={col.id} className="flex items-center gap-2 px-2 py-1.5 hover:bg-slate-50 rounded cursor-pointer group">
                                <input
                                  type="checkbox"
          ...
          <table className="w-full text-left border-collapse">
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-1.5">
                        {col.label}
                        <div className={`flex flex-col -space-y-1 ${isSorted ? "text-blue-600" : "text-slate-300 opacity-0 group-hover:opacity-100"}`}>
                          <ChevronUp className={`w-2.5 h-2.5 ${isSorted && sortState.sortOrder === "ASC" ? "" : "opacity-40"}`} />
                          <ChevronDown className={`w-2.5 h-2.5 ${isSorted && sortState.sortOrder === "DESC" ? "" : "opacity-40"}`} />
                        </div>
                      </div>
                    </th>
                  );
            ...
            <tbody className="divide-y divide-slate-100 text-sm">
              {loading ? (
                  ...
                  <tr>
                      <td colSpan={visibleColumns.length} className="px-6 py-12 text-center text-red-500 bg-red-50/20">
                          {error}
                      </td>
                  </tr>
              ) : jobs.length === 0 ? (
                  ...
                  jobs.map((job) => (
                  <tr key={`${job.job_id}-${job.start_time}`} className="hover:bg-slate-50/80 transition-colors group">
                      {isColumnVisible("job") && (
                      ...
                      {isColumnVisible("project") && (
                        <td className="px-6 py-4 text-slate-600 font-medium">{job.project_id || "-"}</td>
                      )}
                      {isColumnVisible("destination") && (
                        <td className="px-6 py-4 text-slate-500 text-xs truncate max-w-[150px]" title={job.destination}>
                          {job.destination || "-"}
                        </td>
                      )}
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2 text-slate-600">
                            ...
                            {job.owners && job.owners.length > 1 && (
                              <span className="text-[10px] bg-slate-100 px-1 rounded">+{job.owners.length - 1}</span>
                            )}
                          </div>
                        </td>
                      )}
                      {isColumnVisible("period") && (
                        <td className="px-6 py-4 text-slate-600 text-xs italic">{job.period || "-"}</td>
                      )}
        ...
        {totalCount > 0 && (
          <div className="p-4 border-t border-slate-100 flex justify-between items-center bg-slate-50/30">
            <div className="text-xs text-slate-500">
              Showing <span className="font-semibold">{Math.min(totalCount, (currentPage - 1) * ITEMS_PER_PAGE + 1)}</span> to <span className="font-semibold">{Math.min(totalCount, currentPage * ITEMS_PER_PAGE)}</span> of <span className="font-semibold">{totalCount}</span> jobs
            </div>
        ...
        <>
          ...
          <div className="fixed inset-y-0 right-0 w-96 bg-white shadow-2xl z-50 flex flex-col border-l border-slate-200 animate-slide-in-right">
            <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              ...
              <button onClick={() => setShowFilter(false)} className="p-2 hover:bg-slate-200 rounded-full transition-colors">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div className="space-y-4">
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-400 uppercase">Identification</label>
                      ...
                      >
                        {filters.owners.length === 0 ? (
                          ...
                          filters.owners.map(o => (
                            <span key={o} className="bg-blue-50 text-blue-700 px-1.5 py-0.5 rounded text-[10px] font-bold flex items-center gap-1">
                              {o}
                              <X 
                                className="w-3 h-3 cursor-pointer hover:text-blue-900" 
                                onClick={(e) => {
                                  e.stopPropagation();
                                  setFilters(prev => ({ ...prev, owners: prev.owners.filter(x => x !== o) }));
                                }}
                              />
                            </span>
                          ))
                          ...
                          <div className="max-h-48 overflow-y-auto p-1">
                            {filteredOwners.length > 0 && (
                              ...
                              >
                                ...
                                <span className="text-xs font-bold text-slate-700">
                                  {tempOwners.length === 0 ? "Select all" : 
                                   tempOwners.length === filteredOwners.length ? `All ${filteredOwners.length} selected` : 
                                   `${tempOwners.length} of ${filteredOwners.length} selected`}
                                </span>
                              </div>
                            )}
                            {filteredOwners.map(owner => (
                              <label key={owner} className="flex items-center gap-2 px-2 py-1.5 hover:bg-slate-50 rounded cursor-pointer group">
                                <input 
                                  type="checkbox"
                                  ...
                                  className="w-3.5 h-3.5 rounded border-slate-300 text-blue-600 focus:ring-blue-500"
                                />
                                <span className="text-xs text-slate-600 group-hover:text-slate-900 truncate">{owner}</span>
                              </label>
                            ))}
                          </div>
                          <div className="p-2 border-t border-slate-100 flex justify-end gap-2 bg-slate-50/50">
                      ...
                      >
                        {filters.projects.length === 0 ? (
                          ...
                          filters.projects.map(p => (
                            <span key={p} className="bg-blue-50 text-blue-700 px-1.5 py-0.5 rounded text-[10px] font-bold flex items-center gap-1">
                              {p}
                              <X 
                                className="w-3 h-3 cursor-pointer hover:text-blue-900" 
                                onClick={(e) => {
                                  e.stopPropagation();
                                  setFilters(prev => ({ ...prev, projects: prev.projects.filter(x => x !== p) }));
                                }}
                              />
                            </span>
                          ))
                          ...
                          <div className="max-h-48 overflow-y-auto p-1">
                            {filteredProjects.length > 0 && (
                              ...
                              >
                                ...
                                <span className="text-xs font-bold text-slate-700">
                                  {tempProjects.length === 0 ? "Select all" : 
                                   tempProjects.length === filteredProjects.length ? `All ${filteredProjects.length} selected` : 
                                   `${tempProjects.length} of ${filteredProjects.length} selected`}
                                </span>
                              </div>
                            )}
                            {filteredProjects.map(project => (
                              <label key={project} className="flex items-center gap-2 px-2 py-1.5 hover:bg-slate-50 rounded cursor-pointer group">
                                <input 
                                  type="checkbox"
                                  checked={tempProjects.includes(project)}
                                  onChange={() => {
                                    setTempProjects(prev => 
                                      prev.includes(project) ? prev.filter(x => x !== project) : [...prev, project]
                                    );
                                  }}
                                  className="w-3.5 h-3.5 rounded border-slate-300 text-blue-600 focus:ring-blue-500"
                                />
                                <span className="text-xs text-slate-600 group-hover:text-slate-900 truncate">{project}</span>
                              </label>
                            ))}
                          </div>
                          <div className="p-2 border-t border-slate-100 flex justify-end gap-2 bg-slate-50/50">
                      ...
                      >
                        {filters.statuses.length === 0 ? (
                          ...
                          filters.statuses.map(s => (
                            <span key={s} className="bg-blue-50 text-blue-700 px-1.5 py-0.5 rounded text-[10px] font-bold flex items-center gap-1">
                              {s}
                              <X 
                                className="w-3 h-3 cursor-pointer hover:text-blue-900" 
                                onClick={(e) => {
                          ...
                          <div className="max-h-48 overflow-y-auto p-1">
                            {filteredStatuses.length > 0 && (
                              ...
                              >
                                ...
                                <span className="text-xs font-bold text-slate-700">
                                  {tempStatuses.length === 0 ? "Select all" : 
                                   tempStatuses.length === filteredStatuses.length ? `All ${filteredStatuses.length} selected` : 
                                   `${tempStatuses.length} of ${filteredStatuses.length} selected`}
                                </span>
                              </div>
                            )}
                            {filteredStatuses.map(status => (
                              <label key={status} className="flex items-center gap-2 px-2 py-1.5 hover:bg-slate-50 rounded cursor-pointer group">
                                <input 
                                  type="checkbox"
                                  ...
                                  className="w-3.5 h-3.5 rounded border-slate-300 text-blue-600 focus:ring-blue-500"
                                />
                                <span className="text-xs text-slate-600 group-hover:text-slate-900 truncate">{status}</span>
                              </label>
                            ))}
                          </div>
                          <div className="p-2 border-t border-slate-100 flex justify-end gap-2 bg-slate-50/50">
