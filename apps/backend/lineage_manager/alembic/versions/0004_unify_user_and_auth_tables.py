...
def upgrade() -> None:
    ...
    op.create_index(
        op.f("ix_user_account_status"), "user_account", ["status"], unique=False
    )

    # 4. Handle user_id (Adding it as a new column for now)
    op.add_column(
        "user_account", sa.Column("user_id", sa.String(length=100), nullable=True)
    )
    ...
    op.create_index(
        op.f("ix_user_account_user_id"), "user_account", ["user_id"], unique=True
    )
...
def downgrade() -> None:
    ...
    op.alter_column(
        "user_account", "login_id", new_column_name="loginId", type_=sa.String(255)
    )
    op.alter_column(
        "user_account", "department", new_column_name="dept", type_=sa.String(255)
    )
