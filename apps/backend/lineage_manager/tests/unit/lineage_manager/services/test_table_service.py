...
class TestTableService:
    ...
    def test_get_table_dependencies_internal(self, service, mock_query_service):
        ...
        mock_query_service.get_table_dependencies.assert_called_once_with("dataset.table")
        assert result == {"status": "success"}
    ...
    async def test_set_table_dependency_success(self, mock_broker, service, mock_command_service):
        mock_command_service.set_table_dependency.return_value = {"status": "success", "job_id": "job1"}
        
        result = await service.set_table_dependency("table", "job1", True)
        
        mock_command_service.set_table_dependency.assert_called_once_with("table", "job1", True)
        mock_broker.publish.assert_called_once_with("trigger_update", result)
        assert result["status"] == "success"

    @pytest.mark.asyncio
    @patch("lineage_manager.services.table_service.broker", new_callable=AsyncMock)
    async def test_bulk_set_table_dependencies(self, mock_broker, service, mock_command_service):
        ...
        mock_broker.publish.assert_any_call("trigger_update", {
            "status": "success",
            "job_id": "job1",
            "table_name": "table",
            "previous_state": None,
