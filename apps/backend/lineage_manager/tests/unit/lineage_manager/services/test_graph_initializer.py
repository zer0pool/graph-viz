...
class TestGraphInitializerService:
    ...
    @pytest.mark.asyncio
    async def test_initialize_success(self, service, mock_job_manager, mock_command_service, mock_query_service):
        # Mock job manager data
        mock_job_manager.get_all_jobs = AsyncMock(return_value=[
    ...
    @pytest.mark.asyncio
    async def test_initialize_partial_failure_with_fallback(self, service, mock_job_manager, mock_command_service, mock_query_service):
        jobs = [
            SchedulingLineage(job_id="job1", name="Job 1", type="SELF-TYPE", status="ENABLED"),
            SchedulingLineage(job_id="job2", name="Job 2", type="SELF-TYPE", status="ENABLED")
        ]
    ...
    @pytest.mark.asyncio
    async def test_initialize_api_error(self, service, mock_job_manager, mock_command_service):
