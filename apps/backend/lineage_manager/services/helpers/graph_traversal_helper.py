...
class GraphTraversalHelper:
    ...
    def _get_neighbors_for_node(
        ...
        if node_type == "job":
            ...
            if direction in ("upstream", "both"):
                for job_obj in uow.job_table_links.get_jobs_by_table_and_io_type(
                    node_id, "output"
                ):
        ...
        while queue:
            ...
            if node_type == "table":
                if direction == "upstream":
                    ...
                    for job in producers:
                        if ("job", job.id) not in visited:
                            ...
                            queue.append(
                                ("job", job.id, job.job_id, depth + 1, full_name)
                            )
                else:
                    # Downstream: find jobs that consume this table
                    consumers = uow.job_table_links.get_jobs_by_table_and_io_type(
                        db_id, "input"
                    )
                ...
                if direction == "upstream":
                    ...
                    inputs = uow.job_table_links.get_tables_by_job_and_io_type(
                        db_id, "input"
                    )
                    for tbl in inputs:
                        if ("table", tbl.id) not in visited:
                            ...
                            queue.append(
                                ("table", tbl.id, tbl.full_name, depth + 1, full_name)
                            )
                else:
                    # Downstream: find tables that this job writes
                    outputs = uow.job_table_links.get_tables_by_job_and_io_type(
                        db_id, "output"
                    )
                    for tbl in outputs:
                        if ("table", tbl.id) not in visited:
    ...
    def find_root_and_leaf_nodes(
        ...
        if upstream_items:
            ...
            root_nodes = [
                item["id"] for item in upstream_items if item["depth"] == max_depth
            ]
        ...
        if downstream_items:
            ...
            leaf_nodes = [
                item["id"] for item in downstream_items if item["depth"] == max_depth
            ]
