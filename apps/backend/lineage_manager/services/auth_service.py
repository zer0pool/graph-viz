...
class AuthService:
    ...
    async def handle_callback(
        ...
        if not stored_nonce:
            ...
            if not stored_state or state != stored_state:
                logger.error(
                    "[Auth] OIDC state mismatch! Stored: %s, Received: %s",
                    stored_state,
                    state,
                )
        ...
        try:
            ...
            logger.info(
                "[Auth] Session established successfully in backend for sub=%s",
    ...
    def get_current_user(self, request: Request) -> Dict[str, Any]:
        ...
        if session_user:
            logger.info(
                f"[Auth][get_current_user] ✅ Session HIT. Found user: {session_user.get('sub')} (email={session_user.get('email')})"
            )
            return session_user
        ...
        logger.info(
            f"[Auth][get_current_user] ⚠️ Session MISS. Checking 'require_signin' config... Value={require_signin}"
        )

        if not require_signin:
            logger.info(
                "[Auth][get_current_user] 🔓 'require_signin' is FALSE. Falling back to ANONYMOUS USER."
            )
            return {
                "sub": "anonymous-user",
                "name": "Anonymous User",
        ...
        logger.info(
            "[Auth][get_current_user] 🔒 'require_signin' is TRUE. Returning None (Client should 401)."
        )
        return None
