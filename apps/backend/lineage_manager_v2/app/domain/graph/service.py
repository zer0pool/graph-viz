...
from app.domain.graph.schemas import GraphNodeCreate, GraphEdgeCreate, LineageResponse, GraphNodeRead, GraphEdgeRead
...
class GraphService:
    ...
    async def get_or_create_node(self, node_type: str, name: str) -> GraphNodeRead:
        ...
        # Create
        new_node = await self.repo.create_node(GraphNodeCreate(node_type=node_type, name=name))
        return GraphNodeRead.model_validate(new_node)

    async def add_dependency(self, source_id: int, target_id: int, type: str = "LINEAGE") -> GraphEdgeRead:
