...
from app.api.graphql.schema import (MetricBreakdown, MetricDimensions,
                                    MetricGroups, MetricStatus, TrendPoint)
...
class AnalyticsService:
    ...
    async def _resolve_active_users(self) -> MetricGroups:
        stats = await self._get_internal_stats()
        count = stats.get("users", {}).get(
            "total", 0
        )  # Mocking active as total for now
        return MetricGroups(
            id=strawberry.ID("active_users"),
            label="Active Users",
            count=count,
    ...
    async def _resolve_failed_24h(self) -> MetricGroups:
        ...
        return MetricGroups(
            id=strawberry.ID("failed_24h"),
            label="Failed (24h)",
            count=5,
            status=MetricStatus.DESTRUCTIVE,
        )

    async def _resolve_running_now(self) -> MetricGroups:
        return MetricGroups(
            id=strawberry.ID("running_now"),
            label="Running Now",
            count=2,
            status=MetricStatus.SUCCESS,
        )
    ...
    async def _resolve_active_users_yoy(self) -> MetricGroups:
        ...
        return MetricGroups(
            id=strawberry.ID("active_users_yoy"),
            ...
            results.append(
                MetricGroups(
                    ...
                    dimensions=MetricDimensions(
                        name=dept
                    ),  # Use name dimension for heatmap labels
                )
            )
        return results
    ...
    async def _resolve_top_visited(self) -> MetricGroups:
        ...
        return MetricGroups(
            id=strawberry.ID("top_visited_pages"),
    ...
    async def _resolve_expiring_soon(self) -> MetricGroups:
        return MetricGroups(
            id=strawberry.ID("expiring_soon"),
            label="Expiring (7d)",
            count=12,
    ...
    async def _resolve_lineage_coverage(self) -> MetricGroups:
        return MetricGroups(
            id=strawberry.ID("lineage_coverage"),
    ...
    async def _resolve_metadata_health(self) -> MetricGroups:
        return MetricGroups(
            id=strawberry.ID("metadata_health"),
    ...
    async def get_dashboard_metrics(self) -> Dict[str, Any]:
        ...
        return {
            "total_jobs": next(
                (m.count for m in metrics if str(m.id) == "total_jobs"), 0
            ),
            "active_users": next(
            ...
            "failed_24h": next(
                (m.count for m in metrics if str(m.id) == "failed_24h"), 0
            ),
        }
    ...
    async def get_top_visited(self) -> Dict[str, Any]:
        ...
        return {"items": items, "window_hours": 168}  # As requested in example
