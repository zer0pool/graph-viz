...
## 1. 구현 요약
`Metrics Manager` 서비스에 실시간 방문 추적(Tracking) 기능을 구현하였습니다. 프론트엔드에서 페이지 이동 시 발생하는 로그를 수집하고, Redis를 활용하여 실시간 인기도 통계를 생성합니다.

## 2. 변경 사항
### 2.1 Backend (Metrics Manager)
...
### 2.2 Frontend
- **API 경로 수정**: `/admin-console/metrics-manager/api/v1/analytics/track` 호출 확인 및 `useTracker.ts`, `analyticsApi.ts` 경로 업데이트 반영
