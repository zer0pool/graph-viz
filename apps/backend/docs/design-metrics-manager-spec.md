...
## 1. Why GraphQL for metrics-manager?
...
| :--- | :--- | :--- | :--- |
| `/api/metrics-manager/*` | `metrics-manager:5004` | GraphQL | Analytics, Metrics, Aggregations |
...
upstream metrics_manager {
    server metrics-manager:5004;
}

server {
    listen 80;
    
    # Metrics Manager (GraphQL Analytics)
    location /api/metrics-manager/ {
        proxy_pass http://metrics_manager/;
...
// GraphQL Client (Metrics & Analytics)
const metricsClient = new GraphQLClient('/api/metrics-manager/graphql', {
  headers: {
...
class Settings(BaseSettings):
    # Service
    SERVICE_NAME: str = "metrics-manager"
    PORT: int = 5004
...
- **Update**: `metrics-manager` worker performs an **UPSERT** on the projection table.
- **Goal**: Latency between Write -> Projection should be < 1 second.

#### ⚠️ IMPLEMENTATION DECISION REQUIRED (Before Development Starts)

**Question**: How will `metrics-manager` receive updates from `lineage-manager`?
...
| **B. Webhook** | `lineage-manager` POSTs to `metrics-manager` endpoint | Real-time, low latency | Coupling, retry logic needed | Medium |
...
- **Query Labeling**: Attach `metrics-manager` service label to BQ queries.
...
app.include_router(graphql_router, prefix="/api/metrics-manager")
```

**Access**: `http://localhost:5004/api/metrics-manager/graphql`
...
celery_app = Celery("metrics_worker", broker=settings.REDIS_URL)
