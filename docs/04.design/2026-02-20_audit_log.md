...
sequenceDiagram
    ...
    loop For each sub-task
        W->>W: Execute Logic (e.g., Stop Job #1)
        alt Success
            W->>D: Record Audit Detail (SUCCESS)
        else Failure
            W->>D: Record Audit Detail (FAIL + Error Msg)
        end
    end
...
#### 2) `audit_details` (Sub-task Specific Logs)
| Column Name | Type | Constraints | Description |
| :--- | :--- | :--- | :--- |
...
| `status` | VARCHAR(20) | NOT NULL | SUCCESS, FAIL, IN_PROGRESS |
| `message` | TEXT | - | Success details or error message |
| `finished_at` | TIMESTAMP | - | Completion time of the individual task |
            ...
            for msg_id, data in msg_list:
                ...
                for target in targets:
                    try:
                        ...
                        db.execute(
                            "INSERT INTO audit_details (parent_id, target_id, status) VALUES (%s, %s, %s)", 
                            (audit_id, target, 'SUCCESS')
                        )
                        s_count += 1
                    except Exception as e:
                        # Record Detail Failure
                        db.execute(
                            "INSERT INTO audit_details (parent_id, target_id, status, message) VALUES (%s, %s, %s, %s)", 
                            (audit_id, target, 'FAIL', str(e))
                        )
                        f_count += 1
...
- **Detailed View (Drill-down)**: Selecting a specific `audit_id` shows each sub-task's success/failure from `audit_details`, with filtering options (e.g., "Show Failed Only").
- **Real-time Updates**: While the page is open, the progress (success_count / total_count) is updated via 5-10 second polling or WebSocket/SSE for live monitoring.
...
When a user toggles a job status, the following three events must be recorded in `audit_details`:

| Event Order | `description` (Frontend) | `status` | Metadata Included |
| :--- | :--- | :--- | :--- |
| **1. Initiation** | `Job status change initiated (Target: my_job_id)` | `SUCCESS` | Start timestamp |
| **2. Completion** | `Job status change completed (New Status: Stopped)` | `SUCCESS` | Finish timestamp |
| **3. Metric** | `Execution metadata: took 1.24s` | `SUCCESS` | Calculated duration |
    ...
    "events": [
      {
        "id": "event-01",
        "timestamp": "00:20:01",
        "description": "Job status change initiated (Daily_Sync)",
        "status": "SUCCESS"
      },
      {
        "id": "event-02",
      ...
      },
      {
        "id": "event-03",
        "timestamp": "00:20:02",
        "description": "Execution metadata: took 1.15s",
        "status": "SUCCESS"
      }
    ]
  }
