...
graph TD
    ...
    subgraph "Backend Stack (Docker Compose: backend)"
        AppManager["lineage-manager (FastAPI)<br/>Port: 5003"]
...
- **web-shell**:
  ...
  - **Nginx Reverse Proxy**: 프론트엔드 코드에서 발생하는 `/api` 요청을 백엔드 서비스(`lineage-manager`)로 전달.
- **web-lineage (MFE)**:
...
- **lineage-manager (Lineage Manager)**:
...
- **app-job-dummy**:
  ...
  - `lineage-manager`의 요청을 받아 가상의 데이터 추출 작업을 수행.
...
3.  **API 요청**: 사용자가 검색을 수행하면, `web-shell` 내의 Nginx 프록시가 요청을 받아 백엔드(`lineage-manager`)로 전달합니다.
