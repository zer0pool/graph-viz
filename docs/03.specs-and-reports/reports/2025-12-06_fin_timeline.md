...
- Added `GET /api/v1/tables/{table_name}/timelines?days=7` which returns dummy `daily_summary` and `hourly_detail` payloads after a guaranteed 1 s delay.  
...
- Introduced `TableTimelinessView` (`static/js/app/ui/timelinesView.js`) to encapsulate chart rendering, loading/error states, and selection handling.  
...
- `PanelController` now wires the new tab:
  - Stores timelines fetch state, listens for tab switches, and calls the API via `ApiClient.fetchTableTimeliness`.  
...
- CSS (`modern-console.css`) gained styles for the timelines cards, placeholders, and chart sizing to match the GCP lineage explorer feel.

## UX Behavior
1. Select a table node → metadata + Trigger Jobs render as before.  
2. Open **Load timeline v2** → spinner shows while fetching timelines data.  
...
## Follow-ups
- Replace dummy API values with real timelines metrics when backend data is available.  
